...
namespace libtp::util::color
{
    ...
    uint16_t blendOverlayRgb565( uint8_t grayVal, uint8_t* rgb )
    {
        ...
        // if ( grayVal <= 0x7f )
        // {
        //     uint32_t grayTimesTwo = 2 * grayVal;

        //     r = ( grayTimesTwo * rgb[0] ) / 255;
        //     g = ( grayTimesTwo * rgb[1] ) / 255;
        //     b = ( grayTimesTwo * rgb[2] ) / 255;
        // }
        // else
        // {
        //     uint32_t multiplier = 2 * ( 255 - grayVal );

        //     r = ( 255 - multiplier * ( 255 - rgb[0] ) ) / 255;
        //     g = ( 255 - multiplier * ( 255 - rgb[1] ) ) / 255;
        //     b = ( 255 - multiplier * ( 255 - rgb[2] ) ) / 255;
        // }
        ...
        // Divide each by 255
        uint32_t r = ( rTimes255 + 1 + ( rTimes255 >> 8 ) ) >> 8;
        uint32_t g = ( gTimes255 + 1 + ( gTimes255 >> 8 ) ) >> 8;
        uint32_t b = ( bTimes255 + 1 + ( bTimes255 >> 8 ) ) >> 8;
